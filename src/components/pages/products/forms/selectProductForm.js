...
const SelectedProduct = (props) => {
  ...
  const [rentedProductsByName, setRentedProductsByName] = useState([]);
  ...
  const _onInputNameChange = (event, value) => {
    ...
    if (isBook) {
      ...
      setRentedProductsByName(productService.getAllRentedProducts());
    }
  };
      ...
      <Grid container spacing={1} className={classes.pageContent}>
        <Grid item xs={12} sm={4}>
          <Autocomplete
            ...
            options={isBook ? availableProductsByName : rentedProductsByName}
